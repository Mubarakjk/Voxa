...
export function WeeklyMissionScreen() {
  ...
  return (
    <ScreenShell padded={false}>
      <ScrollView contentContainerStyle={styles.scroll} onLayout={() => void refresh()} showsVerticalScrollIndicator={false}>
        <ScreenHeader
          showBack
          title="Weekly mission"
          subtitle={`${daysLeft} days remaining · +${mission.xpReward} XP`}
        />
        ...
        <GlassCard style={styles.card}>
          <View style={styles.cardInner}>
            ...
            <VoxaText variant="caption" color="textMuted" style={styles.status}>
              {pct}% complete · Status: {mission.status}
            </VoxaText>
              ...
              {mission.tasks.map((task) => (
                <View key={task.id} style={styles.taskRow}>
                  ...
                  <View style={styles.taskCopy}>
                    <VoxaText variant="body" color={task.skipped ? 'textMuted' : 'text'} style={styles.taskLabel}>
                      {task.label}
                    </VoxaText>
                    <VoxaText variant="caption" color="textMuted" style={styles.taskMeta}>
                  ...
                  {!mission.completed && mission.status !== 'abandoned' && !task.skipped ? (
                    <View style={styles.taskActions}>
                      <Pressable
                        disabled={busy}
                        hitSlop={6}
                        onPress={() => void run(() => service.completeTask(profile.id, task.id))}>
                      ...
                      <Pressable
                        disabled={busy}
                        hitSlop={6}
                        onPress={() => void run(() => service.undoTask(profile.id, task.id))}>
                      ...
                      <Pressable
                        disabled={busy}
                        hitSlop={6}
                        onPress={() => void run(() => service.skipTask(profile.id, task.id))}>
                        <VoxaText variant="caption" color="textMuted">
                          Skip
                        </VoxaText>
                      </Pressable>
            ...
            <View style={styles.actions}>
              {mission.status === 'pending' ? (
                <PremiumButton
                  label="Start mission"
                  disabled={busy}
                  onPress={() => void run(() => service.start(profile.id))}
                />
              ) : null}
              {mission.status === 'active' ? (
                <PremiumButton
                  ...
                  disabled={busy}
                  onPress={() =>
                    Alert.alert('Abandon mission?', 'No guilt — restart anytime this week.', [
              ...
              {mission.status === 'abandoned' ? (
                <PremiumButton
                  ...
                  disabled={busy}
                  onPress={() => void run(() => service.restart(profile.id))}
                />
              ) : null}
            </View>
          </View>
...
const styles = StyleSheet.create({
  ...
  title: { lineHeight: 24 },
  body: { lineHeight: 22 },
  status: { lineHeight: 18, marginTop: spacing.xs },
  progressTrack: {
    height: 6,
    borderRadius: 3,
  ...
  taskLabel: { lineHeight: 22, flexShrink: 1 },
  taskMeta: { lineHeight: 18 },
  taskActions: {
    flexShrink: 0,
    flexDirection: 'row',
    flexWrap: 'wrap',
